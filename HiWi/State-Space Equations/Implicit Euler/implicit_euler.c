...
void implicit_euler(
    ...
    x_buf   = matrix(0, num_body * 2 - 1, 0, t_end / dt); 
    u_buf   = malloc((num_body * 2 - 1) * sizeof(double));
    A_buf   = matrix(0, num_body * 2 - 1, 0, num_body * 2 - 1);
    B_buf   = matrix(0, num_body * 2 - 1, 0, num_body * 2 - 1);

    /*Allocate memory for velocity SLE*/
    A_vel   = matrix(0, num_body - 1, 0, num_body - 1);
    b_vel   = malloc((num_body - 1) * sizeof(double));
    ...
    free_matrix(A_vel, 0, num_body - 1, 0, num_body - 1);
    free(b_vel);
    free(v_1_new);

    /*Free buffer memory*/
    free_matrix(x_buf, 0, num_body * 2 - 1, 0, t_end / dt);
    free(u_buf);
    free_matrix(A_buf, 0, num_body * 2 - 1, 0, num_body * 2 - 1);
    free_matrix(B_buf, 0, num_body * 2 - 1, 0, num_body * 2 - 1); 
