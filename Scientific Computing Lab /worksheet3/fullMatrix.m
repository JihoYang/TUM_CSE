...
function result = fullMatrix(forcing, boundary)
% FULLMATRIX solves the discrete Poisson equation on a rectangular grid. 
% It uses a full matrix representation of the Laplacian and MATLAB's
% backslash solver.
% 
...
%              represent the Laplacian operator.
    ...
    A = full(make_laplacian_matrix_sparse(nx,ny));
    ...
    result          = struct();
    result.method   = 'Full matrix';
    result.size     = nx;
    result.solution = T;
    result.runtime  = runtime;
    result.storage  = numel(A) + numel(t) + numel(b);
