...
double ***matrix( int nrl, int nrh, int ncl, int nch, int npl, int nph)
{
   int i,j;
   ...
   double ***pce = (double ***) malloc((size_t)( np * sizeof(double**)));

   if( pce == 0)  ERROR("Storage cannot be allocated");

   for (i = 0; i <= np; i++){

        pce[i] = (double **) malloc((size_t)( nrow * sizeof(double*)));
        
        for (j = 0; j < nrow; j++){

                pce[i][j] = (double*) malloc((size_t)( ncol * sizeof(double)));
...
double **matrix_2d( int nrl, int nrh, int ncl, int nch){
    ...
    /* first entry of the array points to the value corrected by the beginning of the column */
    pArray[0] = pMatrix - ncl; 
...
void free_matrix_2d( double **m, int nrl, int nrh, int ncl, int nch)
{
...
void init_matrix( double ***m, int nrl, int nrh, int ncl, int nch, int npl, int nph, double a)
{
   int i,j,k;
...
void init_matrix_2d( double **m, int nrl, int nrh, int ncl, int nch, double a)
{
